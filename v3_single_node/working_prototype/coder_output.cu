...
int main() {
    int N = 256; // Matrix size (power of 2)
...
// 1. Save the code as matrixMul.cu
// 2. Compile: nvcc matrixMul.cu -o matrixMul
// 3. Run: ./matrixMul
// Ensure you have the NVIDIA CUDA Toolkit installed and configured correctly.
// Adjust N to change the matrix size.
// The block size can be tuned for optimal performance.
