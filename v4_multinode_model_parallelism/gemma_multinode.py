...
class Agent:
    ...
    def save(self, output_text: str, filename: str, overwrite: bool = False):
        ...
        header = f"=== {self.role_name} Output (rank {os.environ.get('LOCAL_RANK', '?')}) ===\n"
...
def run_chain(task_description: str, output_filename: str):

    analyst_text = analyst.generate(task_description)
    print("=== Analyst says (rank {os.environ.get('RANK', '?')}) ===")
    print(analyst_text + "\n")
    analyst.save(analyst_text, output_filename, overwrite=True)

    coder_text = coder.generate(analyst_text)
    print("=== Coder writes (rank {os.environ.get('RANK', '?')}) ===")
    print(coder_text + "\n")
    coder.save(coder_text, output_filename)

    builder_text = builder.generate(coder_text)
    print("=== Builder generates CMakeLists.txt (rank {os.environ.get('RANK', '?')}) ===")
    print(builder_text + "\n")
    builder.save(builder_text, output_filename)

    tester_text = tester.generate(coder_text)
    print("=== Tester generates unit tests (rank {os.environ.get('RANK', '?')}) ===")
    print(tester_text + "\n")
    tester.save(tester_text, output_filename)

if __name__ == "__main__":
    run_chain(
            task_description="Describe how to implement matrix multiplication in CUDA.",
            output_filename="chain_of_agents_output.txt"
    )
